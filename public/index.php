...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete_business') {
    ...
    if ($connectionError) {
        $_SESSION['flash_error'] = 'Veritabanı bağlantısı olmadığı için silme işlemi gerçekleştirilemedi.';
        header('Location: ' . $redirectBase);
        exit;
    }
    ...
    try {
        ...
        $_SESSION['flash_success'] = 'İşletme "' . $business['name'] . '" ve ilişkili veriler başarıyla silindi.';
    } catch (Throwable $exception) {
        $_SESSION['flash_error'] = 'İşletme silinemedi: ' . $exception->getMessage();
    }
...
foreach ($businessStats as $stats) {
    ...
    $totalReplied += (int)($stats['replied'] ?? 0);
    $totalPending += (int)($stats['pending'] ?? 0);
}

foreach ($businesses as $business) {
    $timestamp = $business['lastCheckedAt'] ?? null;
    if (($business['connectionStatus'] ?? '') === 'connected') {
        $connectedCount++;
    }
    if ($timestamp && (!$latestCron || strtotime((string)$timestamp) > strtotime((string)$latestCron))) {
        $latestCron = $timestamp;
    }
}
...
<body class="app-body">
    ...
    <div class="app-aurora app-aurora--secondary"></div>
    <div class="app-shell">
            ...
            <div class="app-header__left">
                <div class="brand">
                    ...
                    <div class="brand__text">
                        ...
                        <span class="brand__subtitle">Gemini destekli yorum orkestrasyon merkezi</span>
                    </div>
                </div>
                <div class="header-meta">
                    <span class="meta-pill">⏱ Cron: 1 dk</span>
                    <span class="meta-pill meta-pill--glow">İşletme: <?= count($businesses) ?></span>
                    <span class="meta-pill">Son Cron: <?= format_datetime($latestCron) ?></span>
        ...
        <main class="content">
            <?php if ($connectionError || $flashError || $success): ?>
            ...
            <section class="metrics-grid">
                <article class="metric-card metric-card--primary">
                    ...
                    <p class="metric-card__label">Toplam Google yorumu</p>
                    <div class="metric-card__footer">Senkronize edilen tüm işletmelerden</div>
                </article>
                <article class="metric-card metric-card--success">
                    <div class="metric-card__icon">✨</div>
                    <div class="metric-card__value"><?= $totalReplied ?></div>
                    <p class="metric-card__label">Gemini ile yanıtlanan yorum</p>
                    <div class="metric-card__footer">Çok dilli yapay zeka cevapları</div>
                </article>
                <article class="metric-card metric-card--warning">
                    <div class="metric-card__icon">⏳</div>
                    <div class="metric-card__value"><?= $totalPending ?></div>
                    <p class="metric-card__label">Bekleyen yanıt</p>
                    <div class="metric-card__footer">Cron, yeni yorumları dakikada bir kontrol eder</div>
                </article>
                <article class="metric-card">
                    <div class="metric-card__icon">🔗</div>
                    <div class="metric-card__value"><?= $connectedCount ?></div>
                    <p class="metric-card__label">Bağlı işletme</p>
                    <div class="metric-card__footer">Toplam kayıt: <?= count($businesses) ?></div>
                </article>
            </section>
                                        ...
                                        <tr class="<?= $selectedBusiness && $selectedBusiness['id'] === $business['id'] ? 'is-active' : '' ?>">
                                            ...
                                            <td>
                                                <div class="actions-stack">
                                                    ...
                                                    <form method="post" class="inline-form inline-form--danger" onsubmit="return confirm('İşletmeyi ve tüm kayıtlı yorumları silmek istediğinden emin misin?');">
                                                        <input type="hidden" name="action" value="delete_business">
                                                        <input type="hidden" name="business_id" value="<?= $business['id'] ?>">
                                                        <div class="inline-form__header">
                                                            <span class="inline-form__title">İşletmeyi Sil</span>
                                                            <p class="inline-form__hint">Silme işlemi ilişkili yorum kayıtlarını da kaldırır.</p>
                                                        </div>
                                                        <button type="submit" class="button button--danger button--full">İşletmeyi Sil</button>
                                                    </form>
                                                </div>
                                            </td>
                                        </tr>
